...
Framework7.prototype.plugins.indexedlist = function (app, params) {
    ...
    function initIndexedList(page) {
        ...
        // var eventsTarget = $$('.list-index');
		
		// var eventsTarget = document.getElementsByClassName('list-index')[0];
		console.log('Eventslength:'+eventsTarget.length);
        if (eventsTarget.length === 0) return;

        var pageContent = $(page.container).find('.thepagecontent');
        buildLetters();
        ...
        function buildLetters() {
            ...
            pageContent.find('.list-group').each(function () {
				...
				console.log('Letter '+_letter+' found!')
            });
